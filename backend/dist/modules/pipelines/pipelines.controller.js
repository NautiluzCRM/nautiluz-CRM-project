...
import { z } from 'zod';
import { createPipeline, createStage, deletePipeline, deleteStage, listPipelines, listStages, updatePipeline, updateStage } from './pipelines.service.js';
...
export const listPipelinesHandler = asyncHandler(async (_req, res) => {
    res.json(await listPipelines());
});
export const createPipelineHandler = asyncHandler(async (req, res) => {
    const body = pipelineSchema.parse(req.body);
    const pipeline = await createPipeline(body);
    res.status(201).json(pipeline);
});
export const updatePipelineHandler = asyncHandler(async (req, res) => {
...
export const deletePipelineHandler = asyncHandler(async (req, res) => {
